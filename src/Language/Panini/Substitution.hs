...
--    rename @n@ to something fresh that doesn't yet occur in @e@ (and also
...
instance Subable Type where
  subst x y = \case
    ...
    TBase n b r
      | n == y -> TBase n b r                            -- 1st case
      | n == x -> let n' = freshName x (y : freeVars r)  -- 2nd case
                      r' = subst n' n r
    ...
    TFun n t1 t2
      | n == y -> TFun n (subst x y t1) t2                 -- 1st case
      | n == x -> let n'  = freshName x (y : freeVars t2)  -- 2nd case
                      t2' = subst n' n t2
...
instance Subable Pred where  
  subst x y = \case
    PTrue -> PTrue
    PFalse -> PFalse    
    PVar n -> if n == y then PVar x else PVar n
